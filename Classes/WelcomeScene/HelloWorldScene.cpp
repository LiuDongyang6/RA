...
bool HelloWorld::init()
{
	...
	// start sprite
	Sprite* startSpriteNormal = Sprite::create("start-up1.png");
	Sprite* startSpriteSelected = Sprite::create("start-down1.png");
...
void HelloWorld::onClick(Ref*, TouchEventType type) {
	switch (type) {
	case TouchEventType::TOUCH_EVENT_BEGAN: // 单机事件开始时触发（按下的时候）
		break;
	...
	case TouchEventType::TOUCH_EVENT_ENDED: // 按下之后，然后松开时触发
		...
		break;
	case TouchEventType::TOUCH_EVENT_CANCELED: // 因为一些特殊情况中断时触发
		break;
	}
}
